export default function BudgetVariationsChart() {
  const chartData = [
    { label: 'Original Budget', value: 3.0, color: '#0F172A' },
    { label: 'Approved Variations', value: 0.35, color: '#10B981' },
    { label: 'Revised Budget', value: 3.35, color: '#0F172A' },
    { label: 'Spend to Date', value: 2.8, color: '#F59E0B' },
    { label: 'Balance Budget', value: 0.55, color: '#10B981' }
  ];

  const maxValue = 3.6;
  const gridLines = [0, 0.72, 1.44, 2.16, 2.88, 3.6];
    ...
    <div className="bg-white mt-6">
      <div className="bg-[#F1F5F9] rounded-lg p-8 border border-gray-100 shadow-sm">
        ...
        <div className="relative" style={{ height: '400px', paddingTop: '40px', paddingBottom: '60px' }}>
          ...
          {/* Bars Container */}
          <div className="absolute left-16 right-0 top-10 bottom-16 flex items-end justify-around px-8">
            {chartData.map((item, index) => {
              const heightPercentage = (item.value / maxValue) * 100;
              return (
                <div key={index} className="flex flex-col items-center justify-end" style={{ width: '70px', height: '100%' }}>
                  {/* Value Label */}
                  <div 
          ...
          {/* X-Axis Labels */}
          <div className="absolute left-16 right-0 bottom-0 flex items-center justify-around px-8" style={{ height: '50px' }}>
            {chartData.map((item, index) => (
              <div key={index} className="text-xs font-semibold text-gray-700 text-center" style={{ width: '70px' }}>
                {item.label}
              </div>
            ))}
          </div>
